...
import { Filter, Download, TrendingUp, TrendingDown, AlertTriangle, Plus, Timer, ClipboardCheck, IndianRupee } from "lucide-react";
import { toast } from "sonner";
import { PageHeader } from "@/components/page-header";
import { Button } from "@/components/ui/button";
import { nodes } from "@/data/hul-mock";
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <KpiCard
          ...
          icon={IndianRupee}
          tone="text-primary"
          delta={<span className="text-emerald-600 flex items-center text-xs"><TrendingUp className="w-3 h-3 mr-1" /> +2.4% vs PW</span>}
        />
        <KpiCard
          label="Avg Coverage"
          value="18.4 Days"
          icon={Timer}
          tone="text-tertiary"
          delta={<span className="text-destructive flex items-center text-xs"><TrendingDown className="w-3 h-3 mr-1" /> −1.2 days risk</span>}
        />
        <KpiCard
          label="QC Hold"
          value="1,242 units"
          ...
          delta={<span className="text-muted-foreground flex items-center text-xs"><span className="w-2 h-2 rounded-full bg-secondary-fixed-dim mr-2" /> 4 nodes impacted</span>}
        />
        <KpiCard
          label="Critical SKUs"
          value="12 SKU"
          ...
          delta={<span className="text-on-surface-variant flex items-center text-xs">Immediate action required</span>}
        />
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="bg-card border border-outline-variant rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="relative h-[400px] bg-surface-container-low">
              <div
                className="absolute inset-0 w-full h-full opacity-40 grayscale"
                style={{
                  backgroundImage:
                    "url('https://lh3.googleusercontent.com/aida-public/AB6AXuAfhamZJqn3exHfzxzbPHv5szOq4rgoyKP_vwOYGU-kHHkpmmUkjZ-BFSJWDGg6SjXln0GzonohUKzu8l_bkDTtAcq2UT2sdh3mUzlDxe9Vetm1vXXLmC6ZJXfFGHemRpEqBwjTD3fb37ExRUouwL3TFKv46Sv1tTuLoN0b7q_V9G0SXp7UPF2_2ouhlePkeoK84YFvWwjOcbVud1SbzFlJ5R3D51DEFyRPy6ewKCjqC8Ry-y7pWTV4m_6RCrX2LE_US4kBOB-uaJI')",
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      <div className={`w-3.5 h-3.5 rounded-full ${dot} ring-4 ring-white/60 animate-pulse`} />
                      <div className="glass-card absolute top-5 left-0 border border-outline-variant rounded-xl shadow-lg p-3 w-56 opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none">
                        ...
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span
                            className={`font-bold ${critical ? "text-destructive" : ""}`}
                          >
                            {n.stockPct}%
                          </span>
                        </div>
                        <div className="w-full bg-surface-container h-1.5 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${critical ? "bg-destructive" : okay ? "bg-emerald-500" : "bg-tertiary"}`}
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-card border border-outline-variant rounded-xl p-5 shadow-sm">
            ...
            <div className="relative w-48 h-48 mx-auto flex items-center justify-center">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="16" fill="transparent" stroke="var(--color-surface-container)" strokeWidth="3.5" />
                <circle cx="18" cy="18" r="16" fill="transparent" stroke="var(--color-primary)" strokeDasharray="65 100" strokeWidth="3.5" strokeLinecap="round" />
                <circle cx="18" cy="18" r="16" fill="transparent" stroke="var(--color-secondary-fixed-dim)" strokeDasharray="20 100" strokeDashoffset="-65" strokeWidth="3.5" />
                <circle cx="18" cy="18" r="16" fill="transparent" stroke="var(--color-destructive)" strokeDasharray="15 100" strokeDashoffset="-85" strokeWidth="3.5" />
              </svg>
          ...
          <div className="bg-card border border-outline-variant rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <AgeRow label="60+ days (obsolete risk)" units="1.1k" pct={12} tone="bg-tertiary" muted />
            </div>
          </div>
          ...
          <div className="bg-card border border-outline-variant rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-4">
              <RiskRow name="Surf Excel Matic 2kg" days={2} tone="destructive" inv={240} demand={2100} avail={10} inTransit={15} shortage={75} />
              <RiskRow name="Rin Advanced Bar" days={9} tone="secondary" inv={1800} demand={2400} avail={45} inTransit={35} shortage={20} />
              <RiskRow name="Horlicks Classic 500g" days={6} tone="destructive" inv={620} demand={1500} avail={30} inTransit={20} shortage={50} />
            </div>
            <Button variant="outline" size="sm" className="w-full mt-4">
              View all coverage alerts
            </Button>
          </div>
        </div>
      </div>
...
function RiskRow({
  ...
  const border = tone === "destructive" ? "border-destructive bg-destructive/5" : "border-secondary-fixed-dim bg-surface-container-low";
  ...
  return (
    <div className={`p-3 rounded-lg border-l-4 ${border}`}>
      ...
      <div className="flex h-3 gap-0.5 rounded-sm overflow-hidden mb-2">
        <div className="bg-emerald-500" style={{ width: `${avail}%` }} title="Available" />
        <div className="bg-secondary-container" style={{ width: `${inTransit}%` }} title="In transit" />
